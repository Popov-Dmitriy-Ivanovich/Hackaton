...
class TestDbConnection(unittest.TestCase):
    ...
    def test_users_table(self):
        ...
        with self.subTest(msg="testing updating"):
            to_update = self.db_connection.session.query(UsersTable).filter_by(
                id=rows[0].id
            )
        ...
        with self.subTest(msg="testing deleting"):
            del_id = (
                self.db_connection.session.query(UsersTable)
                .filter_by(id=rows[1].id)
                .first()
            )
    ...
    def test_professions_talbe(self):
        ...
        with self.subTest(msg="testing updating"):
            to_update = (
                self.db_connection.session.query(ProfessionsTable)
                .filter_by(id=rows[0].id)
                .first()
            )
    ...
    def test_login_data_table(self):
        ...
        with self.subTest(msg="testing updating"):
            to_update = (
                self.db_connection.session.query(LoginDataTable)
                .filter_by(login=rows[0].login)
                .first()
            )
    ...
    def test_courses_table(self):
        ...
        with self.subTest(msg="testing updating"):
            to_update = (
                self.db_connection.session.query(CoursesTable)
                .filter_by(course_name=rows[0].course_name)
                .first()
            )
        ...
        with self.subTest(msg="testing deleting"):
            del_id = (
                self.db_connection.session.query(CoursesTable)
                .filter_by(course_name=rows[1].course_name)
                .first()
            )
            self.db_connection.session.delete(del_id)
            self.db_connection.session.commit()
