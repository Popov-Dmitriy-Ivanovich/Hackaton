    ...
    def test_registration(self):
        for log_data in self.test_data:
            with self.subTest(msg=f'testing{log_data}'):
                body = {'name':log_data[0],'login':log_data[1],'password':log_data[2]}
                if log_data[0]==None:
                ...
                if log_data[0]==None and log_data[1]==None and log_data[2]==None:
                    body = {}
                self.assertTrue(log_data[3].check_ex(request('post',URL,json=body)),msg=request('post',URL,json=body).text)
                if log_data[3]==expect_ok:
                    self.assertNotEqual(db_handler.get_login_entity(log_data[1]),None)
                    self.assertEqual(db_handler.get_login_entity(log_data[1]).login,log_data[1])
                    self.assertEqual(db_handler.get_login_entity(log_data[1]).password,log_data[2])
                elif log_data[3]!=expect_422 and log_data[3]!=expect_logex:
                    self.assertEqual(db_handler.get_login_entity(log_data[1]),None,msg=log_data)
