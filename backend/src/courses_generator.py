...
from db.db_classes import connect_db, LoginDataTable, UsersTable, FavouritesTable,CoursesTable, ProfessionsTable
...
class CoursesGenerator():
    ...
    def get_professions_and_courses_id (self, professions: list[str]):
        ...
        for prof in professions:
            prof_entity = self._connection.session.query(ProfessionsTable).filter_by(name=prof).first()
            if(prof_entity == None):
                raise Exception(f'profession {prof} is not found in database')
            prof_ids.append(prof_entity.id)
        for i in prof_ids:
            courses_entities = self._connection.session.query(CoursesTable).filter_by(profession_id = i).all()
    ...
    def guess_professions_by_login(self, login: str) -> list[str]:
        login_entity = self._connection.session.query(LoginDataTable).filter_by(login = login).first()
        if (login_entity == None):
            raise Exception('login data not found')
        user_entity = self._connection.session.query(UsersTable).filter_by(id = login_entity.user_id).first()
        if (user_entity == None):
    ...
    def guess_professions(self, vk_id: str,access_token: str, favourites: list[str]) -> list[str]:
    ...
    def _get_user_favourites(self, user_id: int) -> list[str]:
        professions_entity = self._connection.session.query(FavouritesTable).filter_by(user_id = user_id).all()
        return list(map(lambda x: x.name,professions_entity))

    def _get_user_vk_id(self, user_id: int):
        return self._connection.session.query(UsersTable).filter_by(id=user_id).first().vk_id

    def _get_user_access_token(self, user_id: int):
        return self._connection.session.query(UsersTable).filter_by(id=user_id).first().access_token
    ...
    def execute(self, login: str):
        ...
        cour_data = []
        for id in prof_ids:
            prof_entity = self._connection.session.query(ProfessionsTable).filter_by(id = id).first()
            if prof_entity == None:
                raise Exception('I hope that never happens')
            prof_data.append(ProfessionData(name = prof_entity.name,description=prof_entity.description))

        for name in cour_names:
            cour_entity = self._connection.session.query(CoursesTable).filter_by(course_name = name).first()
            if prof_entity == None:
                raise Exception('I hope that never happens')
            cour_data.append(CourseData(name = cour_entity.course_name,description=cour_entity.description))
        print(prof_data)
        print(cour_data)
