...
class VkAuth(object):
    ...
    def add_user_vk_data_to_db(self):
        ...
        conn = connect_db()
        login_entity = (
            conn.session.query(LoginDataTable).filter_by(login=user_login).first()
        )
        if login_entity == None:
            raise Exception("user not found")
        user_entity = (
            conn.session.query(UsersTable).filter_by(id=login_entity.user_id).first()
        )
        if user_entity == None:
            raise Exception(
                "something wrong with database login exists, but user doesnt"
            )
