...
def process_favourites(form_res: ProfileFormResult):
    try:
        conn = db.connect_db()
        user = conn.session.query(db.UsersTable).filter_by(id = conn.session.query(db.LoginDataTable).filter_by(login = form_res.login).first().user_id).first()
        user_fav = conn.session.query(db.FavouritesTable).filter_by(user_id=user.id).all()
        for i in user_fav:
